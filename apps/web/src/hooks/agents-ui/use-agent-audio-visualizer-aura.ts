...
export function useAgentAudioVisualizerAura(
  ...
  useEffect(() => {
    switch (state) {
      ...
      case 'speaking':
        setSpeed(40);
        animateScale(0.25, DEFAULT_TRANSITION);
        animateAmplitude(0.5, DEFAULT_TRANSITION);
  ...
  // Support raw AnalyserNode for custom WebSocket audio streams
  useEffect(() => {
    ...
    const updateVolume = () => {
      ...
      // Map volume dynamically. We use a non-linear curve to make standard voice frequencies jump more
      const rawVol = Math.min(1.0, avg / 100.0); 
      
      if (!scaleMotionValue.isAnimating()) {
        animateScale(0.25 + 0.15 * rawVol, { duration: 0 });
      }
  ...
  }, [state, analyser, scaleMotionValue, animateScale]);
